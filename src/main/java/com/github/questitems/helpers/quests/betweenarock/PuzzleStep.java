...
public class PuzzleStep extends DetailedQuestStep
{
	...
	public PuzzleStep(github.questitems github.questitems, ItemRequirement item)
	{
		super(github.questitems, "Click the highlighted boxes to move the pieces to the correct spots.", item);
	...
	public void makeWidgetOverlayHint(Graphics2D graphics, github.questitemsPlugin plugin)
	{
		...
		for (Map.Entry<Integer, Integer> entry : highlightButtons.entrySet())
		{
			...
			if (widget != null)
			{
				graphics.setColor(new Color(getgithub.questitems().getConfig().targetOverlayColor().getRed(),
					getgithub.questitems().getConfig().targetOverlayColor().getGreen(),
					getgithub.questitems().getConfig().targetOverlayColor().getBlue(), 65));
